...
import { Sparkle, Buildings, Storefront, Lightning, X } from "@phosphor-icons/react";
...
export default function PainelContas({
  ...
  function atualizarContaLocal(clienteId: string, contaId: string, alteracoes: Partial<ContaMeta>) {
    setClientes((atual) =>
      atual.map((c) =>
        c.id !== clienteId
          ? c
          : {
              ...c,
              smartads_contas_meta: c.smartads_contas_meta.map((conta) =>
                conta.id === contaId ? { ...conta, ...alteracoes } : conta
              ),
            }
      )
    );
  ...
  return (
    <div className="flex flex-col gap-6">
                    ...
                    {cliente.smartads_contas_meta.map((conta) => (
                      <div key={conta.id} className="selo-vidro px-3 py-2 text-xs text-neutral-400">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            <Lightning size={11} weight={conta.boost_automatico_ativo ? "fill" : "regular"} />
                            Boost automático{conta.boost_automatico_ativo ? " · ligado" : ""}
                          </button>
                        </div>
                      </div>
                    ))}
      ...
      {boostContaSelecionada && (
        <ModalBoostAutomatico
          clienteId={boostContaSelecionada.clienteId}
          conta={boostContaSelecionada.conta}
          onFechar={() => setBoostContaSelecionada(null)}
          onSalvo={(alteracoes) =>
            atualizarContaLocal(boostContaSelecionada.clienteId, boostContaSelecionada.conta.id, alteracoes)
          }
        />
      )}
    </div>
 ...
 * montagem que essa automação ainda não cobre. */
function ModalBoostAutomatico({
  clienteId,
  conta,
  onFechar,
  onSalvo,
  ...
  conta: ContaMeta;
  onFechar: () => void;
  onSalvo: (alteracoes: Partial<ContaMeta>) => void;
}) {
